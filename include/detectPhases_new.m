...
function [data] = detectPhases_new(data_in)
    %% LOAD 

    data = data_in;%data = readtable(data_in);

    sig = data.("GyroZ (deg/s)");
    ...
    for i=2:L-1
        ...
        if MSw_found
            ...
            if (sig_new(i,1) < sig_new(i-1,1)) && (sig_new(i,1) < sig_new(i+1,1)) % IC condition
    ...
    data = [data, array2table(sig_new, 'VariableNames',{'gyro_z_filt','gyro_z_heavy_filt', 'ID'})];
end
