classdef myLpmsSer < handle
    ...
    methods
        ...
        function ret = checkConnection(obj)
            %ret false if errors in connectionState are found
            if(isempty(obj.serConn) && obj.isSensorConnected)
                ret = false;
                obj.isSensorConnected = false;
                disp("error in connection state: state true when no connection is available; corrected");
                return;
            elseif(isempty(obj.serConn) && ~obj.isSensorConnected)
                disp("no device connected");
                ret = true;
                return;
            else
                status = getpinstatus(obj.serConn);
                if(obj.isSensorConnected && status.CarrierDetect)
                    disp("device connected");
                    ret = true;
                    return;
                elseif(obj.isSensorConnected && ~status.CarrierDetect)
                    disp("device disconnected suddenly, connection deleted");
                    ret = false;
                    obj.disconnect();
                    return;
                end
        ...
        function ret = connect(obj, serPort, baudrate)
            ...
            if(obj.isSensorConnected)
                disp("device already connected");
                return;
            end
            disp("try connection...");
            %try to connect
            try
                obj.serConn = serialport(serPort,baudrate,"Timeout",5);
            catch err
                disp(err);
            end
            %check if connection is successful
            if(~isempty(obj.serConn))
        ...
        function ret = sendData(obj, command, length) %lenght as number of bytes
            ...
            try
                write(obj.serConn, txBuffer, "uint8");
                ret = true;
            catch
                ret = false;
                obj.disconnect();
...
%         end
        ...
        function readCallbackFcn(obj, hObject, eventdata)
            ...
            if(obj.isSensorConnected)
                try
                    n = obj.serConn.NumBytesAvailable;
                    disp(n);
                    if (n > 0)
                        data = read(obj.serConn, n, "uint8");
                        disp("parsing...");
